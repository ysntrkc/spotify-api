...
async function fetchTopTracks(timeRange) {
    try {
        ...
        const response = await fetch(`http://localhost:8000/top-tracks?time_range=${timeRange}&limit=${limit}`);
        const result = await response.json();
        if (result.status === 'success') {
...
async function fetchTopSingers(timeRange) {
    try {
        ...
        const response = await fetch(`http://localhost:8000/top-singers?time_range=${timeRange}&limit=${limit}`);
        const result = await response.json();
        if (result.status === 'success') {
            const singersList = document.getElementById("singers-list");
